...
```
workspace-root/
├── Microsoft/pxt/                    # Core MakeCode framework (pxt-core)
├── Microsoft/pxt-common-packages/    # Shared cross-target APIs and look in the base and core libraries.
```
...
#### pxt-microbit 
**Repository**: https://github.com/microsoft/pxt-microbit
**Local Path**: `.` (workspace root)
**Package Name**: `pxt-microbit`
**Changes should go here for:**
- Target configuration (`pxtarget.json`, `targetconfig.json`)
- Microbit-specific library extensions in `libs/`
- Microbit-specific documentation (`docs/`, `docfiles/`)
- Editor customizations (`editor/`)
- Microbit themes and styling (`theme/`)
- Build and deployment scripts (`scripts/`)
...
2. **Microbit target configuration**
...
cd ../pxt-microbit && npm run service  # Starts the microbit development server
```
...
- **Microbit-specific features**: Usually pxt-microbit
...
#### Configuration Files for arcade:
...
#### Configuration Files for microbit:
- `pxtarget.json` (pxt-microbit): Main target configuration - **released** version
- `targetconfig.json` (pxt-microbit): Live configuration served between releases
- `package.json` files: Dependencies and build scripts in each repo
- `pxt.json` files: Library/package definitions throughout `libs/` directories
...
#### Microbit Target (pxt-microbit/):
- `pxtarget.json`, `targetconfig.json` - Target configuration
- `libs/` - Microbit-specific extensions
- `docs/`, `docfiles/` - Microbit documentation
- `editor/` - Editor customizations
- `theme/` - Microbit-specific styling
...
- **pxt-microbit**: Miicrobit-specific target - most localized changes
